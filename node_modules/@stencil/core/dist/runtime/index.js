...
const registerStyle = (scopeId, cssText) => {
    ...
    if (platform.supportsConstructibleStylesheets) {
            ...
            if (!appliedStyles.has(scopeId)) {
                if (buildConditionals.BUILD.hydrateClientSide && styleContainerNode.host && (styleElm = styleContainerNode.firstElementChild) && styleElm.tagName === 'STYLE') {
                    ...
                    if (platform.cssVarShim) {
                        styleElm = platform.cssVarShim.createHostStyle(hostElm, scopeId, style);
...
const attachStyles = (elm, cmpMeta, mode) => {
    const styleId = addStyle((buildConditionals.BUILD.shadowDom && platform.supportsShadowDom && elm.shadowRoot)
        ? elm.shadowRoot
...
const setAccessor = (elm, memberName, oldValue, newValue, isSvg, flags) => {
    ...
    if (buildConditionals.BUILD.vdomClass && memberName === 'class' && !isSvg) {
    ...
    else if (buildConditionals.BUILD.vdomListener && memberName.startsWith('on') && !platform.isMemberInElement(elm, memberName)) {
        // Event Handlers
        // so if the member name starts with "on" and the 3rd characters is
        // a capital letter, and it's not already a member on the element,
        // then we're assuming it's an event listener
        if (platform.isMemberInElement(elm, utils.toLowerCase(memberName))) {
            // standard event
            // the JSX attribute could have been "onMouseOver" and the
            // member name "onmouseover" is on the element's prototype
            // so let's add the listener "mouseover", which is all lowercased
            memberName = utils.toLowerCase(memberName.substring(2));
        }
        else {
            // custom event
            ...
            memberName = utils.toLowerCase(memberName[2]) + memberName.substring(3);
        }
        if (oldValue) {
        ...
        const isProp = platform.isMemberInElement(elm, memberName);
        ...
        if (newValue == null || newValue === false) {
            if (isXlinkNs) {
                elm.removeAttributeNS(XLINK_NS, utils.toLowerCase(memberName));
            }
            else {
            ...
            if (isXlinkNs) {
                elm.setAttributeNS(XLINK_NS, utils.toLowerCase(memberName), newValue);
            }
            else {
...
const createElm = (oldParentVNode, newParentVNode, childIndex, parentElm) => {
    ...
    if (utils.isDef(newVNode.$text$)) {
        ...
        newVNode.$elm$ = (buildConditionals.BUILD.isDebug || buildConditionals.BUILD.hydrateServerSide) ? platform.doc.createComment(`slot-reference:${hostTagName}`) : platform.doc.createTextNode('');
    }
    else {
        // create element
...
const addVnodes = (parentElm, before, parentVNode, vnodes, startIdx, endIdx) => {
    ...
    let childNode;
    if (buildConditionals.BUILD.shadowDom && containerElm.shadowRoot && utils.toLowerCase(containerElm.tagName) === hostTagName) {
...
const renderVdom = (hostElm, hostRef, cmpMeta, renderFnResults) => {
    ...
    hostTagName = utils.toLowerCase(hostElm.tagName);
...
const scheduleUpdate = async (elm, hostRef, cmpMeta, isInitialLoad) => {
    ...
    if (buildConditionals.BUILD.taskQueue) {
        platform.writeTask(() => updateComponent(elm, hostRef, cmpMeta, instance, isInitialLoad));
    }
    else {
    ...
    if ((buildConditionals.BUILD.lazyLoad || buildConditionals.BUILD.lifecycle || buildConditionals.BUILD.lifecycleDOMEvents) && !elm['s-al']) {
        ...
        if (!(hostRef.$flags$ & 512 /* hasLoadedComponent */)) {
            ...
            if (buildConditionals.BUILD.lazyLoad && !ancestorComponent) {
                ...
                if (!buildConditionals.BUILD.hydrateServerSide) {
                    setTimeout(() => platform.plt.$flags$ |= 2 /* appLoaded */, 999);
                }
    ...
    return (ev) => {
        if (buildConditionals.BUILD.lazyLoad) {
            if (hostRef.$lazyInstance$) {
                ...
                return hostRef.$onReadyPromise$.then(() => hostRef.$lazyInstance$[methodName](ev)).catch(platform.consoleError);
            }
        }
        else {
        ...
        if (childNodeType) {
            ...
            if (childIdSplt[0] === hostId || childIdSplt[0] === '0') {
                childVNode = {
                    ...
                    $depth$: childIdSplt[2],
                    $index$: childIdSplt[3],
                    $tag$: utils.toLowerCase(node.tagName),
                    $elm$: node
                };
...
const initializeComponent = async (elm, hostRef, cmpMeta, hmrVersionId, Cstr) => {
    ...
    if ((buildConditionals.BUILD.lazyLoad || buildConditionals.BUILD.style) && (hostRef.$flags$ & 256 /* hasInitializedComponent */) === 0) {
        ...
        if (buildConditionals.BUILD.lazyLoad) {
            ...
            Cstr = await platform.loadModule(cmpMeta, hostRef, hmrVersionId);
...
const setContentReference = (elm, contentRefElm) => {
    ...
    contentRefElm = elm['s-cr'] = platform.doc.createComment(crName);
...
const bootstrapLazy = (lazyBundles, options = {}) => {
    ...
    platform.plt.$resourcesUrl$ = new URL(options.resourcesUrl || '/', platform.win.location.href).href;
...
const createEvent = (ref, name, flags) => {
    ...
    return {
        emit: (detail) => elm.dispatchEvent(new (buildConditionals.BUILD.hydrateServerSide ? platform.win.CustomEvent : CustomEvent)(name, {
